...
describe('exampleRouter', () => {
  ...
  app.use('/ai-factory-kaizen/v1', exampleRouter(eventService));

  it('serves the public endpoint without authentication', async () => {
    const res = await request(app).get('/ai-factory-kaizen/v1/public');
    expect(res.status).toBe(200);
    expect(res.body.message).toContain('public');
  });

  it('exposes resource links at the index', async () => {
    const res = await request(app).get('/ai-factory-kaizen/v1');
    expect(res.status).toBe(200);
    expect(res.body._links.public.href).toBe('/ai-factory-kaizen/v1/public');
  });
});
